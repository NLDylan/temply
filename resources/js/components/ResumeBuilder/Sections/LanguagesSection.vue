  ...
  >
    <div class="grid gap-4">
      <div
        ...
        :key="language.id"
        class="grid gap-3 rounded-2xl border border-border/60 bg-muted/30 p-4 sm:grid-cols-[minmax(0,1fr),220px]"
      >
        <div class="grid gap-3">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <Globe2 class="size-4 text-brand" />
              <Input v-model="language.name" placeholder="Language" class="h-9 w-auto bg-background/80" />
            </div>
          ...
          <div class="h-2 w-full overflow-hidden rounded-full bg-background/70">
            <div
              ...
              :style="{
                width:
                  language.fluency === 'Native' ? '100%' : language.fluency === 'Fluent' ? '85%' : language.fluency === 'Professional' ? '70%' : '50%',
              }"
            />
          </div>
        </div>
